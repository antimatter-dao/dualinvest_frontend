...
const getRiskStatementListItems = (subject: Subject) => {
  const listItems = {
    [Subject.DualInvest]: [
  ...
  return (
    <Box padding="32px 24px" display="grid" gap={29}>
      ...
      <Box fontSize={{ xs: 14, md: 16 }} marginLeft="-24px">
        <StyledOrderList>
          {getRiskStatementListItems(subject).map((li, idx) => (
            <li key={idx}>{li}</li>
          ))}
        </StyledOrderList>
      </Box>
...
const getFAQItems = (subject: Subject) => {
  const items = {
    [Subject.DualInvest]: [
      {
        ...
        details: (
          <AccordionDetailText sx={{ fontSize: { xs: 14, md: 16 } }}>
            Antimatter Dual Investment is an advanced options derivative based on a decentralised protocol. The product
            has a &quot;market-neutral, returns guaranteed&quot; feature, where the yield is clear and fixed at the time
            of purchase, while the settlement currency is uncertain. At maturity, the settlement currency depends on the
            outcome of the settlement price at maturity compared to the strike price.
          </AccordionDetailText>
        )
      },
      {
        ...
        details: (
          <AccordionDetailText sx={{ fontSize: { xs: 14, md: 16 } }}>
            <p>
              When a product is &quot;exercised&quot;, the subscription amount and yields will be swapped at the strike
              price in the alternative currency.
            </p>
            <p>
            ...
            <p>
              When a subscription is &quot;unexercised&quot;, the subscription amount and yields will not be transferred
              into the alternative currency and the user will receive the currency they invested.
            </p>
            <p>
...
export function FAQ({ subject }: { subject: Subject }) {
  ...
  return (
    <Box>
      ...
      <Box mt={28}>
        {getFAQItems(subject).map(({ summary, details }, idx) => (
          <Accordion
            key={idx}
            summary={summary}
            details={details}
